 ...
 *
 * @log:
 * - 2014-07-08 spolu  Open new tab at initial load #10
 * - 2014-06-27 spolu  Fix tabs race issue #4
...
var tabs = function(spec, my) {
  ...
  update_tabs = function(value) {
    ...
    my.last.tabs.forEach(function(t, i) {
      ...
      if(t.type === 'new_tab') {
        ...
        common.log.out('[tabs] tabs_new ' + t.tab_id + ' [' + t.url + '] (' + t.load + ')');
        ...
        if(url_for_state(my.core_tabs[t.tab_id].state) !== t.url &&
          ...
          common.log.out('[tabs] tabs_load_url ' + t.tab_id + ' [' + t.url + '] (' + 
                         my.core_tabs[t.tab_id].last_load + ' < ' + t.load + ')');
          my.core_tabs[t.tab_id].last_load = t.load;
    ...
    Object.keys(my.core_tabs).forEach(function(tab_id) {
      if(!tabs[tab_id] && tab_id !== my.new_tab.tab_id) {
        common.log.out('[tabs] tabs_close ' + tab_id);
    ...
    if(my.last_active !== active) {
      ...
      common.log.out('[tabs] tabs_show ' + active);
  ...
  core_state_handler = function(state) {
    Object.keys(state).forEach(function(tab_id) {
      ...
      if(!my.core_tabs[tab_id]) {
        ...
        /* Transit load information. */
        my.last.tabs.forEach(function(t, i) {
          if(t.tab_id === tab_id) {
            my.core_tabs[tab_id].last_load = t.load;
          }
        });
  ...
  action_load = function(url) {
    if(my.last.tabs.length > 0) {
      ...
      common.log.out('[tabs] action_load ' + tab_id + ' [' + url + '] (' + 
                     my.last.tabs[my.last.active].load + ')');
      push_payload({
        type: 'load',
        tab_id: tab_id,
        url: url
